...
/**
  ...
  */
object JournalDao {
    def getAll(orderedBy: Int): Seq[Journal] = {
    val conn = ConnectionUtil.getConnection();
    Using.Manager { use =>
        var stmt = use(conn.prepareStatement("SELECT * FROM project0.journal ORDER BY entry_id;"))
        if(orderedBy == 1){
            stmt = use(conn.prepareStatement("SELECT * FROM project0.journal ORDER BY entry_date;"))
        }
        stmt.execute()
    ...
    def search(entryID: Int): Seq[Journal] = {
    val conn = ConnectionUtil.getConnection();
    Using.Manager { use =>
    ...
    def search(entryDate: Date): Seq[Journal] = {
        val conn = ConnectionUtil.getConnection();
        Using.Manager { use =>
